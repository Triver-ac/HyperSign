...
class SignLanguageDataset(torch.utils.data.Dataset):
    def __init__(self, dataset_cfg, split):
        ...
        self.translate = dataset_cfg.get('translate', False)
    ...
    def __getitem__(self, idx):
        ...
        data = {
            k: v for k, v in self.annotation[idx].items() if k in [
                'name',
                'gloss',
                'text',
                'num_frames',
        ...
        if self.translate:
            raw = u'，。？！：“”'
            mapped = u',。?!:“”'
            # table = {ord(f): ord(t) for f, t in zip(raw, mapped)}
            data['text'] = data['text'].translate({ord(f): ord(t) for f, t in zip(raw, mapped)})
        return data
