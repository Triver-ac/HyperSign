...
def load_batch_video(
        zip_file, names, num_frames, transform_cfg, dataset_name, is_train,
...
def load_video(zip_file, name, num_frames, transform_cfg, dataset_name, is_train):
    ...
    if dataset_name.lower() in ['csl-daily', 'phoenix-2014t', 'phoenix-2014']:
        if dataset_name.lower() == 'csl-daily':
            ...
            # image_path_list = ['{}@fullFrame-210x260px/{}/images{:04d}.png'.format(zip_file, name, fi)
            #                    for fi in range(1, num_frames + 1)]
            image_path_list = ['{}/fullFrame-210x260px/{}/images{:04d}.png'.format(zip_file, name, fi)
                               for fi in range(1, num_frames + 1)]
        ...
        sequence = [
            read_img(
                image_path_list[i], dataset_name,
                csl_cut=transform_cfg.get('csl_cut', True),
                csl_resize=transform_cfg.get('csl_resize', [320, 320])
