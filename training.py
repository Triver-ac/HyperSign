...
# from modelling.model_v import build_model
...
def evaluate_and_save(
    ...
    if ckpt_queue.full():
        ...
        if sort_key(last_score) <= sort_key(score):
            ...
            # current ckpt is worse than the worst ckpt in queue, put the worst ckpt back to queue.
...
def add_args():
    ...
    parser.add_argument("--model-dir", type=str, default="as_cfg")
...
def main():
    ...
    for epoch_no in range(start_epoch, total_epoch):
        ...
        for step, batch in enumerate(train_dataloader):
            # print(torch.cuda.memory_summary())  # 打印CUDA内存的详细使用情况
            if cfg['training']['amp']:
