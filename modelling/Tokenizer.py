...
class TextTokenizer(BaseTokenizer):
    def __init__(self, tokenizer_cfg):
        ...
        if self.level == 'word':
            ...
            self.tokenizer = MBartTokenizer.from_pretrained(**tokenizer_cfg)  # tgt_lang
    ...
    def generate_decoder_inputs(self, input_ids):
        decoder_inputs = shift_tokens_right(
            input_ids,
    ...
    def __call__(self, input_str):
        if self.level == 'sentencepiece':
            ...
            outputs['input_ids'] = pruned_input_ids.clone()
            ...
            outputs['text_length'] = torch.tensor(raw_outputs['length'], dtype=torch.long)
