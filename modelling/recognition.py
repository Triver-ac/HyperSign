...
class RecognitionNetwork(torch.nn.Module):
    def __init__(
            self, cfg, input_type, transform_cfg, input_streams=['rgb']
    ) -> None:
        super().__init__()
        logger = get_logger()
        self.cfg = cfg
        
        self.layer_norm = torch.nn.LayerNorm(normalized_shape=1664)  # 指定输入特征的维度
        ...
        if self.input_type == 'video':
            if 'rgb' in input_streams and 'keypoint' not in input_streams:
                if 's3d' in cfg:
            ...
            elif 'keypoint' in input_streams and 'rgb' not in input_streams:
        ...
        if 'visual_head' in cfg:
            if 'rgb' in input_streams:
                ...
                if cfg['pyramid']['rgb'] is not None and 'multi' in cfg['pyramid']['rgb']:
                    if cfg['pyramid']['rgb'] == 'fused_multi_head':
                        num_remain_heads = cfg['s3d']['use_block']
                    else:
                        num_remain_heads = cfg['s3d']['use_block'] - 1
                    dims = [64, 192, 480, 832]
                    ...
                    for i in range(num_remain_heads):
                        ...
                        self.visual_head_remain.append(
                            VisualHead(cls_num=len(self.gloss_tokenizer), **cfg['visual_head'])
                        )
            else:
            ...
            if 'keypoint' in input_streams:
                ...
                if cfg['pyramid']['pose'] is not None and 'multi' in cfg['pyramid']['pose']:
                    num_remain_heads = cfg['s3d']['use_block'] if cfg['pyramid']['pose'] == 'fused_multi_head' else \
                        cfg['s3d']['use_block'] - 1
                    dims = [64, 192, 480, 832]
                    if cfg['pyramid']['version'] == 'v2':
                        num_levels = cfg['pyramid'].get('num_levels', 3)
        ...
        if 'pretrained_path_rgb' in cfg and 'rgb' in input_streams:
            ...
            if  (not hasattr(RecognitionNetwork, 'visual_backbone')) and (not hasattr(RecognitionNetwork, 'visual_backbone_twostream')):
                neq_load_customized(self.visual_head, head_dict, verbose=False)
                logger.info('Load visual_head for rgb from {}'.format(cfg['pretrained_path_rgb']))
            elif self.visual_backbone is not None and self.visual_backbone_twostream is None:
            ...
            elif self.visual_backbone is None and self.visual_backbone_twostream is not None:
                ...
                logger.info('Load visual_backbone_twostream.rgb_stream and visual_head for rgb from {}'.format(
                    cfg['pretrained_path_rgb']))
            else:
                logger.info('No rgb stream exists in the network')

        if 'pretrained_path_keypoint' in cfg and 'keypoint' in input_streams :
            ...
            if (not hasattr(RecognitionNetwork, 'visual_backbone_keypoint')) and (not hasattr(RecognitionNetwork, 'visual_backbone_twostream')):
                neq_load_customized(self.visual_head_keypoint, head_dict, verbose=False)
                logger.info('Load visual_head for pose from {}'.format(cfg['pretrained_path_keypoint']))
                ...
                neq_load_customized(self.visual_head_keypoint, head_dict, verbose=False)
                logger.info(
                    'Load visual_backbone and visual_head for keypoint from {}'.format(cfg['pretrained_path_keypoint']),
                    verbose=False
                )
            elif self.visual_backbone_keypoint == None and self.visual_backbone_twostream != None:
        ...
        self.recognition_loss_func = torch.nn.CTCLoss(
            blank=self.gloss_tokenizer.silence_id,
            zero_infinity=True,
            reduction='sum'
        )
        ...
        if is_train:
            if sgn_videos != None:
                ...
                i, j, h, w = torchvision.transforms.RandomResizedCrop.get_params(
                    ...
                    ratio=(
                        self.transform_cfg.get('aspect_ratio_min', 3. / 4),
    ...
    def forward(
            self, is_train, gloss_labels, gls_lengths,
        ...
        if self.input_type == 'video':
            ...
            with torch.no_grad():
                ...
                sgn_videos, sgn_heatmaps = self.augment_preprocess_inputs(is_train=is_train, sgn_videos=sgn_videos,
                                                                          sgn_heatmaps=sgn_heatmaps)
            ...
            print("fuse_method",self.fuse_method)
            if self.fuse_method == 'empty':
                print("len(self.input_streams) ",len(self.input_streams) )
                ...
                if 'rgb' in self.input_streams:
                    ...
                    if self.cfg['pyramid']['rgb'] == 'multi_head':
                        for i in range(len(self.visual_head_remain)):
                            head_ops = self.visual_head_remain[i](
                    ...
                    if self.cfg['pyramid']['pose'] == 'multi_head':
                        for i in range(len(self.visual_head_keypoint_remain)):
                            head_ops = self.visual_head_keypoint_remain[i](
                                x=s3d_outputs['fea_lst'][i],
                                mask=s3d_outputs['sgn_mask'][i],
                                valid_len_in=s3d_outputs['valid_len_out'][i]
                            )
                        ...
                        for i in range(self.cfg['s3d']['use_block'] - 2):
                            head_ops = self.visual_head_keypoint(
                                x=s3d_outputs['fea_lst'][i],
                                mask=s3d_outputs['sgn_mask'][i],
                                valid_len_in=s3d_outputs['valid_len_out'][i]
                            )
                ...
                if 'doublehead' in self.fuse_method:
                    ...
                    head_outputs['ensemble_last_gloss_logits'] = (
                            head_outputs['fuse_gloss_probabilities']
                            + head_outputs['rgb_gloss_probabilities']
                            + head_outputs['keypoint_gloss_probabilities']
                    ).log()
                else:
                    raise ValueError
                head_outputs['ensemble_last_gloss_probabilities_log'] = \
                    head_outputs['ensemble_last_gloss_logits'].log_softmax(2)
                head_outputs['ensemble_last_gloss_probabilities'] = \
                    head_outputs['ensemble_last_gloss_logits'].softmax(2)

                if self.cfg['pyramid']['rgb'] == 'multi_head' and self.cfg['pyramid']['pose'] == 'multi_head':
                    head_outputs['ensemble_early_gloss_logits'] = (aux_prob['rgb'][-1] + aux_prob['keypoint'][
                        -1]).log()  # (aux_prob['rgb'][2]+aux_prob['keypoint'][2]).log()
                    head_outputs['ensemble_early_gloss_probabilities_log'] = \
                        head_outputs['ensemble_early_gloss_logits'].log_softmax(2)
                    head_outputs['ensemble_early_gloss_probabilities'] = \
                        head_outputs['ensemble_early_gloss_logits'].softmax(2)
            else:
                raise ValueError
        ...
        if self.fuse_method == 'empty':
            ...
            if 'head_weight' in self.cfg['pyramid']:
                self.cfg['pyramid']['head_weight_rgb'] = self.cfg['pyramid']['head_weight_keypoint'] = \
                    self.cfg['pyramid']['head_weight']
            for k in ['rgb', 'keypoint', 'fuse']:
                ...
                if k in aux_prob_log:
                    for i in range(len(aux_prob_log[k])):
                        outputs[f'recognition_loss_{k}'] += self.cfg['pyramid'][
                                                                f'head_weight_{k}'] * self.compute_recognition_loss(
                            gloss_labels=gloss_labels, gloss_lengths=gls_lengths,
        ...
        elif 'doublehead' in self.fuse_method:
            assert 'rgb' in self.input_streams and 'keypoint' in self.input_streams
            for k in ['rgb', 'keypoint']:
                if f'{k}_gloss_probabilities_log' in head_outputs:
                    outputs[f'recognition_loss_{k}'] = self.compute_recognition_loss(
                        gloss_labels=gloss_labels, gloss_lengths=gls_lengths,
                        gloss_probabilities_log=head_outputs[f'{k}_gloss_probabilities_log'],
