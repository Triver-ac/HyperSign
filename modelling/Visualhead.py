...
class VisualHead(torch.nn.Module):
    def __init__(
            ...
            ff_kernelsize=3, pretrained_ckpt=None, is_empty=False, freeze=False,
            plus_conv_cfg={}, ssl_projection_cfg={},FusionNet={}, type = "RGB"
    ):
        super().__init__()
    ...
    def load_from_pretrained_ckpt(self, pretrained_ckpt):
        ...
        for k, v in checkpoint.items():
            if 'recognition_network.visual_head.' in k:
                new_key = k.replace('recognition_network.visual_head.', '')
                if new_key in self.state_dict() and self.state_dict()[new_key].shape == v.shape:
                    load_dict[new_key] = v
                else:
                    logger.info(f"Skipping loading parameter {new_key}: checkpoint shape {v.shape}, model shape {self.state_dict()[new_key].shape if new_key in self.state_dict() else 'not found in model'}")
        self.load_state_dict(load_dict, strict=False)
        logger.info('Loaded Visual Head from pretrained ckpt {}, skipped mismatched parameters.'.format(pretrained_ckpt))
    ...
    def forward(self, x, mask, valid_len_in=None):
        ...
        if self.is_empty is False:
            if not self.freeze:
                ...
                x = x.transpose(1, 2)
                x = self.plus_conv(x)
                x = x.transpose(1, 2) #gloss_feature
            else:
                with torch.no_grad():
        ...
        gloss_probabilities = logits.softmax(2) #gloss_probabilities
