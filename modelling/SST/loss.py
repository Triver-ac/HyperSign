...
class ClipInfoCELoss(_Loss):
    ...
    #    return loss
    def forward(self, logits_per_image, logits_per_text, type="0"):
        if type == "0":
            ...
            bs, l_bs = logits_per_image.shape
            bs=int(bs/2)
            labels = torch.zeros(2*bs, 2*bs).cuda()
            indices = torch.arange(2*bs).cuda()
            labels[indices, indices] = 1
            labels[indices, (indices+bs) % (2*bs)] = 1  # 处理循环匹配
