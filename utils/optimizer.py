...
def build_optimizer(config: dict, model) -> Optimizer:
    ...
    if "decoder" in config['learning_rate'] or "gaussian" in config['learning_rate']:
        # default
        # mapper:
        # translation:
        # decoder:
        # gaussian:
        logger.info('learning rate default={}'.format(base_lr))
        for name, parm in model.named_parameters():
            lr_ = base_lr
            for key, lr in config['learning_rate'].items():
                if key in name:
                    lr_ = lr
                    if key == "decoder":
                        lr_ = config['learning_rate']['decoder']
                    if key == "gaussian":
                        lr_ = config['learning_rate']['gaussian']
            parameters.append({'params': parm, 'lr': lr_})
        for key, lr in config['learning_rate'].items():
            logger.info('learning rate {}={}'.format(key, lr))
    else:
        for name, child in model.named_children():
            ...
            for m, lr in config['learning_rate'].items():
                if m in name:
                    lr_ = lr
            logger.info('learning rate {}={}'.format(name, lr_))
            parameters.append({'params': child.parameters(), 'lr': lr_})
